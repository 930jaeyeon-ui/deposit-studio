...
function NotificationRules() {
  ...
  return <PageLayout><div className="shell"><section className="panel"><div className="section-heading"><div><span className="eyebrow">PARSING RULES</span><h1>알림 파싱 규칙</h1><p>앱 패키지명을 키로 내용에서 입금자와 금액을 추출합니다.</p></div><button className="primary-button" onClick={()=>open(null)}>추가</button></div>
    {message&&<p className="form-message error">{message}</p>}<div className="table-wrap"><table><thead><tr><th>앱 패키지명</th><th>내용 정규식</th><th>관리</th></tr></thead><tbody>{items.map(item=><tr key={item.packageName}><td><code>{item.packageName}</code></td><td><code>{item.contentPattern}</code></td><td><button onClick={()=>open(item)}>수정</button> <button onClick={()=>remove(item.packageName)}>삭제</button></td></tr>)}</tbody></table></div>
    {editing&&<div className="dialog-backdrop" onMouseDown={event=>event.target===event.currentTarget&&setEditing(null)}><form className="dialog-card" onSubmit={save}><h2>{editing.packageName?'규칙 수정':'규칙 추가'}</h2><label>앱 패키지명<input value={form.packageName} onChange={e=>setForm({...form,packageName:e.target.value})} required/></label><label>내용 정규식<textarea rows="7" value={form.contentPattern} onChange={e=>setForm({...form,contentPattern:e.target.value})} required/></label><p><code>(?&lt;donor&gt;...)</code>와 <code>(?&lt;amount&gt;...)</code> 캡처 그룹이 필요합니다.</p><div className="dialog-actions"><button type="button" onClick={()=>setEditing(null)}>취소</button><button className="primary-button">저장</button></div></form></div>}
  </section></div></PageLayout>;
}
